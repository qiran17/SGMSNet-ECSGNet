...
class ESpatial_attn(nn.Module):
    def __init__(self, dim):
        ...
        self.conv3 = DepthwiseConv2d(dim * 2, dim, kernel_size=3, stride=1, padding=1, dilation=1)
...
if __name__ == '__main__':
    ...
    block = EMSAFusion(dim=32).to(device)
    x = torch.rand(1, 32, 224, 224).cuda()
    y = torch.rand(1, 32, 224, 224).cuda()
    output = block(x, y)
